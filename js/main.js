...
//換HOME
if ($(window).width() < 768) {
    $('.home').attr('src', './media/images/home-sm.png')
}


//換圖
if ($(window).width() < 576) {
    var titleImg = '<img src="./media/images/t-2-2-sm.png" alt="睡覺洗腎白天照常工作">'
...
$('.count-num').on('click', function() {
    ...
    countSwitch = true;
    let noColor = '<img src="./media/images/icon1-4-3.png" alt="人物小圖示">'
...
function count(num, sec) {
    ...
    let fullColor = '<img src="./media/images/icon1-4-1.png" alt="人物小圖示">'
    let halfColor = '<img src="./media/images/icon1-4-2.png" alt="人物小圖示">'

    var m = Math.floor(num / 10) + 1;
    var n = num - (m - 1) * 10;
    for (i = 0; i < m; i++) {
        if (n == 0) {
            $('.icon-box:nth-child(' + i + ')').html(fullColor);
        } else {
            $('.icon-box:nth-child(' + (i + 1) + ')').html(fullColor);
        }
        if (n >= 2 && n < 8) {
            $('.icon-box:nth-child(' + m + ')').html(halfColor);

        } else if (n >= 8) {
            $('.icon-box:nth-child(' + m + ')').html(fullColor);
        }
    }
...
$('.submit').on('click', function() {
    var chart = $('.mob-show-chart').offset().top - 200;
    if ($(window).width() < 992) {
        $("html, body").animate({ scrollTop: chart }, 500, 'swing');
    }
});
...
$('.submit').on('click', function() {
    ...
    var addGuide = '<p class="chart-guide">點擊其他區塊，看更多腎友收入變化</p>'

    if ($(window).width() < 992) {
        $('.mob-show-chart').html(showChart).append(addGuide);
        $('.mob-show-chart>div').append(showText);

    } else {
        $('.show-chart').html(showChart).append(addGuide);
...
gsap.utils.toArray(".g-alone").forEach(function(section) {
    var media = section;
    gsap.from(media, {
        opacity: 0,
        y: 40,
